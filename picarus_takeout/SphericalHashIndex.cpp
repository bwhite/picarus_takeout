...
namespace Picarus {
SphericalHashIndex::SphericalHashIndex(const unsigned char *hashes, int *indeces, int num_hashes, int num_bytes, const std::vector<std::string> &labels, int max_results) : num_hashes(num_hashes), num_bytes(num_bytes), labels(labels), max_results(max_results) {
...
SphericalHashIndex::~SphericalHashIndex() {
    delete [] hashes;
    delete [] indeces;
...
std::vector<std::pair<double, std::string> > *SphericalHashIndex::query_index(const unsigned char *hash, int size) {
    ...
    bitand_cmap_lut16(hashes, hash, temp_bitand, num_bytes, num_hashes, 1);
    for (i = 0; i < num_hashes; ++i)
        max_valid_ind = place_dist_in_results(i, temp_hamming[i] / (double)temp_bitand[i], result_indeces, result_dists, max_results, max_valid_ind);
